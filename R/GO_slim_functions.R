...
getnames_goslim <- function(gslim){
 ...
 #gslim <- get_ontology("C://common_laptop//R-files//GOslim//goslim_generic.obo")
...
annotate_with_go <- function(plist){
  ...
  go_proteins <- names(tempgo) # we lose some proteins.
...
go_slim_annotation <- function(mylist,obo){
  ...
  for (i in 1:length(names(gostuff))){
    ...
    if (go_mf$Count ==0 && go_bp$Count==0 && go_cc$Count ==0){
      cat("\n")
      #cat("\n","mf ",go_mf$Count, "bp ", go_bp$Count, "cc",go_cc$Count)
    }else{
      cat("\n mf ",go_mf$Count, "\n bp ", go_bp$Count, "\n cc",go_cc$Count)
