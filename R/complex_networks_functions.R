...
build_network <- function(ppi){
  ...
  ppi_net <- igraph::as.undirected(ppi_net); 
  ppi_net <- igraph::simplify(ppi_net)  # remove duplicates and self-loops
  ppi_net <- delete_isolates(ppi_net)
  ppi_net <- igraph::delete.vertices(igraph::simplify(ppi_net), igraph::degree(ppi_net)==0)
...
# Table used in paper, of discovered potential targets
# takes list of candidates, network statistics and cutoff for defining a target as parameters
make_table <- function(targettype,gs1,cutoff){
